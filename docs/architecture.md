...
| D25 | **建置工具選 Gradle，不用 Maven**：4 個 module（common/collector/worker/api）共用一個 root `build.gradle.kts`，用 `subprojects { }` 區塊一次套用 Java 21 toolchain、Lombok、測試設定給全部 module。**誠實說明這不是「Maven 做不到」的技術必然性**——Maven 的 multi-module reactor build 一樣能做到「多模組共用一個 repo、各自獨立打包」，D7 講的是 monorepo vs polyrepo vs monolith 這個決策，不是 Gradle vs Maven 的比較。真正的理由是：(1) Kotlin DSL 寫跨模組共用設定（`subprojects{}`）比 Maven parent POM + XML 繼承更直接、能寫真正的邏輯判斷；(2) 現代 Spring Boot 3.x greenfield 專案業界慣例上比較常預設 Gradle；(3) 使用者對 Gradle 較熟悉。這三條都是「偏好/慣例」層級，不是不可替代的硬需求 | 誠實記錄比硬掰一個「Gradle 優於 Maven」的技術理由更有價值——這個決策本身經不起「Maven 不是也有 multi-module 嗎」這種追問，記錄下來避免之後（例如面試被問到）誤以為這是有深思熟慮的技術選型 | 用 Maven 重寫 build 設定（沒有技術理由要換，純粹是為了「證明」什麼而做的無謂遷移工作） |
