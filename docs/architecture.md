...
```
Scheduler ──► List Scraper ──► [jobs.discovered] ──► Detail Fetcher ──► [jobs.raw] ──► Normalizer ──► PostgreSQL
(@Scheduled)  (per-source          Kafka topic        查 PG 決定抓/不抓      Kafka topic     冪等 upsert        │
               adapter)                               集中限速/重試                          + 快照 insert      │
                                                                                              │               │
                                                                                        [jobs.events] ◄───────┘
                                                                                          Kafka topic   (upsert 時發現新缺/變更)
                                                                                              │
                                                                                              ▼
                                                                                       Discord Notifier
REST API ◄── PostgreSQL（查詢現況與歷史）──► 未來前端看板
```
...
| D3 | 兩段式爬蟲：list scraper（發現）＋ detail fetcher（抓全文） | detail 貴、list 便宜；限速集中在 fetcher；單筆重試；平台改版只修薄薄的 scraper | 單段式每輪全抓 detail（request 爆量）；scraper 直連 DB（職責過厚） |
